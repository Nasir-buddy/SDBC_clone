  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-6 md:flex-row">
        <img
          className="h-auto w-full max-w-[300px] rounded-md object-cover shadow-lg transition-shadow hover:shadow-[8px_17px_38px_2px_rgba(101,86,205,1)] md:h-[40vh]"
          src={`https://image.tmdb.org/t/p/original/${info.details.poster_path || info.details.backdrop_path}`}
          alt={`${info.details.name || info.details.title} poster`}
        />
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-black text-white sm:text-4xl md:text-5xl">
            ...
            <small className="ml-2 text-xl font-bold text-zinc-300 sm:text-2xl">{info.details.first_air_date.split('-')[0]}</small>
          </h1>
          ...
          <div className="mt-3 mb-4 flex flex-wrap items-center gap-3 text-white sm:gap-5">
            ...
            <h2>{info.details.first_air_date}</h2>
            <h2>{info.details.genres.map((item) => item.name).join(', ')}</h2>
            <h2>{info.details.episode_run_time?.[0]} min</h2>
          </div>
          ...
          <h2 className="mt-4 mb-2 text-xl text-white">TV Show Translated</h2>
      ...
      {/* Seasons */}
      <div className="mt-8">
        <h2 className="mb-4 text-2xl font-bold text-white sm:text-3xl md:text-4xl">Seasons</h2>
          ...
          {info.details.seasons.length > 0 ? (
            info.details.seasons.map((season, index) => (
              <div key={index} className="flex-shrink-0 w-36 sm:w-48">
                <img
                  className="h-48 sm:h-64 w-full rounded-md object-cover shadow-lg transition-shadow hover:shadow-[8px_17px_38px_2px_rgba(101,86,205,1)]"
                  src={`https://image.tmdb.org/t/p/original/${season.poster_path}`}
                  alt={`${season.name} poster`}
                />
                <h3 className="mt-2 text-lg font-semibold text-white truncate">{season.name}</h3>
              </div>
            ))
          ) : (
            <p className="text-white text-lg">No seasons to show.</p>
          )}
        </div>
      </div>
